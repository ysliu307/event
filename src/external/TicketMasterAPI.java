...
public class TicketMasterAPI implements ExternalAPI{
	...
	private static final String API_KEY = "Gh9GBvSrfXAOtLqE4pWfuwChLwHWD38n";

	// search function, send HTTP request and get response
	@Override
	...
	// 2. add a print function to print json array return from ticketmaster for dubug
	...
	// helper function
	// 1.url-encoding before add it to http request
		...
		for (int i = 0; i < events.length(); i++) {
			...
			if (venue != null) {
				...
				if (!venue.isNull("location")) {
					...
					builder.setLongitude(getNumericFieldOrNull(location, "lontitue"));
				}
			}
